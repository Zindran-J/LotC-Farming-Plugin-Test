...
public class beetroots implements LootTable {
    private Collection<ItemStack> loot = new ArrayList<ItemStack>();
    @Override
    public @NotNull Collection<ItemStack> populateLoot(@Nullable Random random, @NotNull LootContext context) {
        int seedCount = 1;
        int cropCount = 1;

        loot.add(new ItemStack(Material.BEETROOT_SEEDS, seedCount));
        loot.add(new ItemStack(Material.BEETROOT, cropCount));
        return List.of();
    }
